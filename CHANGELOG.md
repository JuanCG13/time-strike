...
- 2026-08-31: `tick` can now issue a bounded, task-and-action-bound, one-shot lease when an action and positive finite ETA fit before the next mandatory check; the host anchors expiry to its monotonic tick-request start, rejects task/action/ETA rebinding, replay and concurrent use, and clamps consumption to its hard deadline, while invalid proposals fail before mutation and legacy clients retain the advisory response.
